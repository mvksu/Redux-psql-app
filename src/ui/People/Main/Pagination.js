...
export default function Pagination({ items, paginate, currentPage }) {
  ...
  return (
    <nav className="flex items-center justify-center py-8">
      {currentPage > 0 && <FaAngleLeft className="text-blue-400 text-2xl" onClick={() => paginate(currentPage-1)}/>}
      ...
      {currentPage < pageNumbers.length && <FaAngleRight className="text-blue-400 text-2xl" onClick={() => paginate(currentPage+1)}/>}
    </nav>
  );
}
